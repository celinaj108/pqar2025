...
aggregate(social_cohesion_rev ~ age_band, data = chs2020_cleaned, FUN = mean, na.rm = TRUE)

# ADDITIONAL: ANOVA
sc_age_lm <- lm(social_cohesion_rev ~ age_band, data = chs2020_cleaned)
anova(sc_age_lm)
...
summarize_variable_clean <- function(data, var, var_label) {
  tab <- data |>
    ...
    mutate(
      percent = round(n / sum(n) * 100, 2),
      Variable = var_label,
      Category = if (var_label == "Birthplace") {
        # leave the text as-is for birthplace
        as.character(!!sym(var))
      } else {
        str_to_title(as.character(!!sym(var)))
      },
      n = n
    ) |>
